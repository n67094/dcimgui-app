...
SDL_AppInit(void **appstate, int argc, char **argv)
{
  ...
  _device = SDL_CreateGPUDevice(SDL_GPU_SHADERFORMAT_SPIRV
  ...
  _window = SDL_CreateWindow(
      WINDOW_TITLE, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_HIGH_PIXEL_DENSITY);
  if (_window == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
  ...
  if (SDL_WindowSupportsGPUPresentMode(
          _device, _window, SDL_GPU_PRESENTMODE_IMMEDIATE)) {
    present_mode = SDL_GPU_PRESENTMODE_IMMEDIATE;
  } else if (SDL_WindowSupportsGPUPresentMode(
                 _device, _window, SDL_GPU_PRESENTMODE_MAILBOX)) {
    present_mode = SDL_GPU_PRESENTMODE_MAILBOX;
  }

  SDL_SetGPUSwapchainParameters(
      _device, _window, SDL_GPU_SWAPCHAINCOMPOSITION_SDR, present_mode);
...
SDL_AppIterate(void *appstate)
{
  ...
  SDL_GPUCommandBuffer *cmd_buffer = SDL_AcquireGPUCommandBuffer(_device);
  if (cmd_buffer == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
